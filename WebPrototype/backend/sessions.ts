...
import { addPlayer, createSessionState, migrateSessionState, type GameSession } from './game';

const alphabet = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
function tableName(code: string) { return 'game_session_' + code; }
function normalizeCode(code: string) {
  return code.trim().toUpperCase().replace(/[^A-Z2-9]/g, '').slice(0, 6);
}
function validName(name: string) {
  const clean = name.trim().slice(0, 24);
  if (clean.length < 2) throw new Error('Name must be 2–24 characters.');
  return clean;
}
function makeCode() {
  let code = '';
...
export async function loadSession(codeInput: string): Promise<{ id: string; session: GameSession } | null> {
  ...
  const { id, ...stored } = items[0];
  return { id, session: migrateSessionState(stored as GameSession) };
}
...
export async function joinSession(codeInput: string, nameInput: string): Promise<{ session: GameSession; playerId: string }> {
  ...
  if (stored.session.campaignStatus !== 'active') throw new Error('That hunt has already ended.');
  const name = validName(nameInput);
