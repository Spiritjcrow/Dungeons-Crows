...
function enemyFor(chapter: number): Combatant {
  ...
  return { id: e.id, name: e.name, hp: e.hp, maxHp: e.hp, defense: e.defense, isEnemy: true };
}
...
function advanceCampaign(session: GameSession, result: string): string {
  ...
  if (session.chapter === 1) {
    ...
    session.log.push({ turn: session.turnNumber, kind: 'system', text: 'Chapter II: The Bone Rookery opens beneath the crypt.' });
  ...
  if (session.chapter === 2) {
    ...
    healBetweenChapters(session);
    session.activeActorId = session.party.find(member => member.hp > 0)?.id || '';
    session.log.push({ turn: session.turnNumber, kind: 'system', text: 'Chapter III: The Black Rook waits upon the broken throne.' });
    return result + ' The purified rookery exhales a stairway of black stone toward the throne.';
  }
  session.campaignStatus = 'victory';
  session.activeActorId = '';
  session.score += 750;
  session.log.push({ turn: session.turnNumber, kind: 'system', text: 'Victory: The Black Rook Oath is broken.' });
...
export function migrateSessionState(input: GameSession): GameSession {
  ...
  next.ritualAttempts = Number.isFinite(legacy.ritualAttempts) ? Number(legacy.ritualAttempts) : 0;
  next.score = Number.isFinite(legacy.score) ? Number(legacy.score) : 0;
  ...
  if (!next.enemy || !next.enemy.name) next.enemy = enemyFor(next.chapter);
  if (next.chapter === 1 && next.enemy.hp <= 0 && next.altarOpened) {
    next.chapter = 2;
    next.enemy = enemyFor(2);
    next.ritualAttempts = 0;
  }
  next.log = Array.isArray(next.log) ? next.log.slice(-40) : [];
  return next;
}
...
export function createSessionState(code: string, name: string, seed: number): { session: GameSession; playerId: string } {
  ...
  const session: GameSession = {
    ...
    score: 0,
    seed,
    party: [{ id: playerId, name, hp: 20, maxHp: 20, defense: 10, isEnemy: false }],
    enemy: enemyFor(1),
    lastNarration: 'The crypt door grinds shut. A black-feathered Warden raises its rusted blade beside a sealed crow altar.',
    log: [{ turn: 0, kind: 'system', text: name + ' entered the Crow Crypt. Chapter I begins.' }],
  };
  return { session, playerId };
}

export function addPlayer(sessionInput: GameSession, name: string): { session: GameSession; playerId: string } {
  const next = migrateSessionState(sessionInput);
  const playerId = next.code + '-p' + (next.party.length + 1) + '-' + Math.floor(Math.random() * 9999).toString(36);
  next.party.push({ id: playerId, name, hp: 20, maxHp: 20, defense: 10, isEnemy: false });
...
export function resolveTurn(sessionInput: GameSession, intent: TurnIntent): TurnResolution {
  const session = migrateSessionState(sessionInput);
  const next = structuredClone(session);
  if (session.campaignStatus !== 'active') return { accepted: false, deterministicResult: 'This hunt has already ended.', diceRolls: [], state: session, narrationSeed: '' };
  ...
  if (intent.actionType === 'attack') {
    if (next.enemy.hp <= 0) return { accepted: false, deterministicResult: next.enemy.name + ' has already fallen. Complete the chapter objective.', diceRolls: [], state: session, narrationSeed: '' };
    const roll = seededD20(next.seed, next.turnNumber, 7 + actorIndex + next.chapter * 13);
    rolls.push(roll);
    const hit = roll + 4 + next.relics.length >= next.enemy.defense;
    if (hit) {
      next.enemy.hp = Math.max(0, next.enemy.hp - 4);
      next.score += 25;
    }
    result = actor.name + (hit ? ' strikes ' + next.enemy.name + ' for 4 damage' : ' misses ' + next.enemy.name) + ' (d20 ' + roll + ').';
  } else if (intent.actionType === 'defend') {
    actor.defendingUntilTurn = next.turnNumber + 1;
    actor.hp = Math.min(actor.maxHp, actor.hp + 2);
    next.score += 5;
    result = actor.name + ' braces behind stone and steel, restores 2 resolve, and gains guard through the next attack.';
  } else if (intent.actionType === 'interact') {
    const expectedTarget = next.chapter === 1 ? 'altar' : next.chapter === 2 ? 'rookery' : 'crown';
    if (intent.targetId !== expectedTarget) return { accepted: false, deterministicResult: 'That is not the active chapter objective.', diceRolls: [], state: session, narrationSeed: '' };
    if (objectiveComplete(next)) return { accepted: false, deterministicResult: 'The chapter objective is already complete.', diceRolls: [], state: session, narrationSeed: '' };
    if (next.chapter === 1) {
      next.altarOpened = true;
      addRelic(next, 'Ember Feather');
      next.score += 75;
      result = actor.name + ' breaks the wax seal. The crow altar wakes and yields the Ember Feather.';
    } else {
      const roll = seededD20(next.seed, next.turnNumber, 211 + actorIndex + next.chapter * 19);
      rolls.push(roll);
      next.ritualAttempts += 1;
      const bonus = next.relics.length;
      const target = next.chapter === 2 ? 10 : 12;
      const success = roll + bonus >= target || next.ritualAttempts >= 3;
      if (success && next.chapter === 2) {
        next.rookeryPurified = true;
        addRelic(next, 'Bone Quill');
        next.score += 100;
        result = actor.name + ' purifies the Bone Rookery and claims the Bone Quill (d20 ' + roll + ' +' + bonus + '; rite ' + next.ritualAttempts + '/3).';
      } else if (success) {
        next.crownBroken = true;
        addRelic(next, 'Shattered Crown');
        next.enemy.defense = Math.max(9, next.enemy.defense - 2);
        next.score += 150;
        result = actor.name + ' breaks the Black Rook Crown, weakening the throne-bound foe (d20 ' + roll + ' +' + bonus + '; rite ' + next.ritualAttempts + '/3).';
      } else {
        result = actor.name + ' advances the rite but does not complete it (d20 ' + roll + ' +' + bonus + ' vs ' + target + '; rite ' + next.ritualAttempts + '/3).';
      }
    }
  } else {
    const spoken = (intent.freeformText || '').trim();
    result = spoken ? actor.name + ' says: “' + spoken.slice(0, 240) + '”' : actor.name + ' calls into the dark, but offers no words.';
  }

  if (next.enemy.hp > 0) {
    const enemyRoll = seededD20(next.seed, next.turnNumber, 101 + actorIndex + next.chapter * 29);
    rolls.push(enemyRoll);
    const guard = (actor.defendingUntilTurn || 0) >= next.turnNumber ? 3 + next.relics.length : 0;
    const attackBonus = next.chapter + 1;
    const damage = 2;
    const hit = enemyRoll + attackBonus >= actor.defense + guard;
    if (hit) actor.hp = Math.max(0, actor.hp - damage);
    result += ' ' + next.enemy.name + (hit ? ' answers for ' + damage + ' damage' : ' attacks, but misses') + ' (d20 ' + enemyRoll + ').';
  }
  ...
  if (next.party.every(member => member.hp <= 0)) {
    ...
    next.activeActorId = '';
    next.log.push({ turn: next.turnNumber, kind: 'system', text: 'Defeat: every adventurer has fallen.' });
    result += ' The last adventurer falls. The hunt is lost.';
  } else {
    next.activeActorId = nextLivingActor(next, actor.id);
    result = advanceCampaign(next, result);
  }
